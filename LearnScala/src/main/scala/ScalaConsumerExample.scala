...
//sbt "run-main ScalaConsumerExample  localhost:2181 group1 topic1 10 0"
...
class ScalaConsumerTest(val stream: KafkaStream[Array[Byte], Array[Byte]], val threadNumber: Int, val delay: Long) extends Logging with Runnable {
  def run {
    ...
    while (it.hasNext()) {
      ...
      val m =new String(msg, "utf-8")
      System.out.println(System.currentTimeMillis() + ",Thread " + threadNumber + ": " + m)
    }
